...
<head>
    ...
    <meta name="keywords"
        content="université, informatique, design, Madagascar, Antananarivo, Andoharanofotsy, master, licence, développement web, technologie" />
...
<body class="no-scroll">
    ...
    <?php if ($page === 'accueil') { ?>
        <!-- PRELOADER -->
        <div id="preloader"
            class="d-flex flex-column justify-content-center align-items-center vh-100 w-100 position-fixed top-0 start-0 z-3">
            ...
            <div class="container-fluid">
                ...
                <div class="d-flex align-items-center gap-2 gap-sm-3 ms-auto ms-lg-0">
                    <button id="theme-toggle"
                        class="btn btn-sm d-flex align-items-center justify-content-center mx-2 text-white"
                        style="width: 38px; height: 38px; flex-shrink: 0" aria-label="Changer de thème">
                        <i id="theme-icon" class="bi bi-moon-fill fs-5"></i>
                    </button>

                    <button
                        class="btn border-0 p-1 ms-1 text-white d-flex justify-content-center align-items-center gap-2"
                        type="button" data-bs-toggle="offcanvas" data-bs-target="#ituOffcanvas"
                        aria-controls="ituOffcanvas" title="Menu">
                ...
                <?php foreach ($offcanvas_links as $link) { ?>
                    ...
                    <?php if (isset($link['sublinks'])) { ?>
                        <div class="nav-item itu-nav-dropdown">
                            ...
                            <a class="nav-link d-flex align-items-center justify-content-between" href="<?= $link['href'] ?>">
                                <?= $link['label'] ?>
                                <i class="bi bi-chevron-down itu-dropdown-arrow ms-3" style="font-size: 1.2rem;"></i>
                            </a>

                            <div class="itu-sub-menu d-flex flex-column">
                                <?php foreach ($link['sublinks'] as $sublink) { ?>
                                    <a class="nav-link sub-nav-link" href="<?= $sublink['href'] ?>">
                                        <span class="fw-bold me-2" style="color: var(--itu-vert);">>_</span>
                                        <?= $sublink['label'] ?>
                                    </a>
                                <?php } ?>
                            </div>
    ...
    <footer class="itu-footer text-white pt-5" data-section-theme="dark">
            ...
            <div class="row gy-5">
                <section class="col-lg-6 pe-lg-5">
                    ...
                    <article class="mt-4 ">
                        ...
                        <a href="mailto:ituniversity@moov.mg"
                            class="d-flex align-items-center mb-3 text-decoration-none contact-link">
                            <i class="bi bi-envelope fs-5 me-3" style="color: var(--itu-vert)"></i>
                            <span class="fs-5">ituniversity@moov.mg</span>
                        </a>
                        <a href="https://maps.google.com/?cid=9140985535758018311&g_mp=Cidnb29nbGUubWFwcy5wbGFjZXMudjEuUGxhY2VzLlNlYXJjaFRleHQ"
                    ...
                    <form action="/subscribe" method="POST" class="newsletter-form">
                        <div class="input-group mb-3">
                            ...
                            <button class="btn btn-itu-vert" type="submit" style="border-radius: 0; padding: 10px 20px"
                                title="Envoyer" aria-label="Envoyer">
                                <i class="bi bi-send-fill"></i>
                            </button>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" value="" id="newsletterCheck" required />
                            <label class="form-check-label text-white opacity-50 small" for="newsletterCheck"
                                style="font-size: 0.75rem"> J'accepte de recevoir des actualités de l'ITU. </label>
                        </div>
                    </form>
                </section>
            </div>
        ...
        <section class="footer-bottom py-4 mt-4"
            style="background-color: #080808; border-top: 1px solid rgba(255, 255, 255, 0.05)">
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-md-6 text-center text-md-start mb-3 mb-md-0">
                        <span style="font-size: 0.85rem; color: #888">&copy; <span id="current-year"></span> IT
                            University. Tous droits réservés.</span>
                    </div>

                    <div class="col-md-6 text-center text-md-end">
                        <span style="font-size: 0.85rem; color: #888">Conçu par <span class="fw-bold text-white">Binary
                                Beasts</span></span>
                    </div>
                </div>
            </div>
    ...
    <button id="scrollToTopBtn" class="btn-itu-top" title="Retour en haut" style="opacity: 0"
